...
export default function FC26Card({ player, asPreview = false, livePhotoUrl = null }) {
  ...
  const CardContent = (
    <div className={`fc-card relative w-64 h-96 bg-gradient-to-br ${bgGradient} ${textColor} p-4 flex flex-col justify-between overflow-hidden ${borderGlow} ${asPreview ? 'shadow-2xl scale-100' : ''}`}>
      ...
      {/* Player Image */}
      <div className="absolute bottom-[110px] left-0 right-0 flex justify-center pointer-events-none z-0 px-2">
      ...
      <div className="z-10 relative mt-auto pt-2 border-t border-current/20 bg-gradient-to-t from-black/10 to-transparent">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm font-bold">
          <div className="flex justify-between">
            <span className="opacity-75">PAC</span> <span>{player.pace || 0}</span>
          </div>
          <div className="flex justify-between">
            <span className="opacity-75">DRI</span> <span>{player.dribbling || 0}</span>
          </div>
          <div className="flex justify-between">
            <span className="opacity-75">SHO</span> <span>{player.shooting || 0}</span>
          </div>
          <div className="flex justify-between">
            <span className="opacity-75">DEF</span> <span>{player.defending || 0}</span>
          </div>
          <div className="flex justify-between">
            <span className="opacity-75">PAS</span> <span>{player.passing || 0}</span>
          </div>
          <div className="flex justify-between">
            <span className="opacity-75">PHY</span> <span>{player.physical || 0}</span>
          </div>
        </div>
      </div>
